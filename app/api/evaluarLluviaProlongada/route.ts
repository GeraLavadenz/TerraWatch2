...
export async function GET() {
  try {
    const fechaHoy = new Date().toISOString().split("T")[0]; // "2025-06-05"
    const lecturasRef = db.ref(`/lecturas/${fechaHoy}`);

    const snapshot = await lecturasRef.limitToLast(30).once("value");
    const datos = snapshot.val();

    if (!datos) {
      return NextResponse.json({ mensaje: "Sin lecturas recientes" }, { status: 404 });
    }
    ...
    return NextResponse.json({
      ...
      ciclos_detectados: ciclosSeguidos,
    });
  } catch (error) {
